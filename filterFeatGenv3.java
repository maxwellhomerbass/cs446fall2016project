 ...
 * */
public class filterFeatGenv3 {
	//original
	static String[] features = new String[] { "srcIP", "srcIPmask", "dstIP",
			"dstIPmask", "srcPortStart", "srcPortEnd", "dstPortStart",
			"dstPortEnd" };
	...
	private static double[] input_to_doubles(String inputLine){
		...
		/**/
		vals[0] = ip_to_dec(parts[0].trim());//srcip
		vals[1] = Integer.parseInt(parts[1]);//mask
		vals[2] = ip_to_dec(parts[2].trim());
		vals[3] = Integer.parseInt(parts[3]);
	 ...
	 * */
	private static double ip_range_min(long ip_dec, int mask){
	 ...
	 * assumption: mask is an int between 0 and 32
	 * */
	private static double ip_range_max(long ip_dec, int mask){
		if(mask==32) return ip_dec;//from an off-by-one error
		long min=(long) ip_range_min(ip_dec,mask);
		int rem = 32-mask;
		return min + ((1<<rem) -2);
	}
	...
	public static void main(String[] args) throws Exception {
		...
		String f2 = "/Users/maxbass/Desktop/cs450/data/easy_test/acl3_seed_1000.filter";
		Instances data = readData(f2);

		ArffSaver saver = new ArffSaver();
		saver.setInstances(data);
		// saver.setFile(new File(args[1]));
		saver.setFile(new File("acl1_seed_1000.arff"));
		saver.writeBatch();
	}
